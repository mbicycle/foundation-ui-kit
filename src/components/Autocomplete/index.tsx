        ...
        <Combobox value={value} onChange={onChange}>
          <div className="relative mt-1">
            <div className="relative w-full cursor-default overflow-hidden">
              <Combobox.Input
                ref={ref}
                className={`w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-3 pr-10 text-left
                ...
                {filteredOptionsArray.length === 0 && query !== "" ? (
                  ...
                  filteredOptionsArray.map((item) => (
                    ...
                    >
                      {({ selected, active }) => (
                        <>
                          <span className={`block truncate text-base ${selected ? "font-semibold" : "font-normal"}`}>
                            {item.name}
                          </span>
                          {selected ? (
                            <span
